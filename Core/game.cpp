...
void Game::createExampleLevel() {
    ...
    // Set creature in last room to boss
...
void Game::displayLevel(std::ostream& display) const {
    ...
    // Display row 1
    for (int i = 0; i < 6; i++) {
        display << theBuilder->getDungeonLevel()->retrieveRoom(1)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(2)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(3)->display().at(i)
                << std::endl;
    }

    // Display row 2
    for (int i = 0; i < 6; i++) {
        display << theBuilder->getDungeonLevel()->retrieveRoom(4)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(5)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(6)->display().at(i)
                << std::endl;
    }

    // Display row 3
    for (int i = 0; i < 6; i++) {
        display << theBuilder->getDungeonLevel()->retrieveRoom(7)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(8)->display().at(i) +
                theBuilder->getDungeonLevel()->retrieveRoom(9)->display().at(i)
                << std::endl;
    }
}
